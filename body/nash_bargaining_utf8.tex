...
博弈论作为在经济学中已经普遍使用的一门数学理论，近年来也被用来解决无线资源管理的问题。
本章我们提出的基于议价的合作博弈模型，
可公平地把系统资源分配给网络中的每个用户。
所提到的纳什议价解是一个公平且优化的资源分配解决方案。
这个解决方案既可以使系统资源充分利用，又能保证每个用户资源分配的公平性。
...
然而，这里存在着一个潜在的冲突和矛盾：卖房者期望一个高的卖出价格的同时，买房者却期望一个低的买入价格。
...
议价理论的出现，使得经济学家们可以用数理的方式解决这些类似的问题。
...
~$\mathbf{d}=(u_1^d, u_2^d)$~表示博弈双方如果不能达成合作协议时，最终双方取得的效用向量。
...
如果想获得唯一的议价解，就需要对上述议价问题附加一些其他的条件（我们将在小节 \ref{subsec:chap_nash:math_analysis} 中继续讨论）。
\begin{figure}[!tb] 
    \centering
...
类似的，如果在一个资源受限的无线网络系统中，存在有~$n$~个用户。
...
令~$\phi = \{ \mathbf{U} \} \subset R^n$~为效用的集合，其中，~$\mathbf{U} =( u_1 =U_1(b_1), \ldots, u_n = U_n(b_n))$~。令~$\mathbf{d} = (d_1, \ldots, d_n)$~是用户的分歧点，其中，~$d_i = U_i(b_{0i})$~。
所以，此博弈问题可定义为 ~$(\mathbf{\phi,d})$~。
接下来我们来描述博弈的细节。
...
另一方面，由于系统的资源是有限的，所以要求满足条件，~$\sum_i^n b_i \le B_{total}$~
...
前面提到，在此情况下，符合帕累托最优的议价解不止一个，经济学家纳什提出如果满足一些条件，
那么就可以确定一个唯一的、更合理的解\cite{Nash_1950}。

这些条件也称之为纳什公理约束。
此处，我们以两个参与者为例来解释。
在一个议价博弈过程中存在有两个参与者。
每个博弈的参与者都追求使自己的效用最大化的选择。
我们把它们分别记为~$u_1$~和~$u_2$~。
...
而它的数学含义是，议价解仅仅依赖于~$\mathbf{u}^*$~邻域中可行集右上边界的形状。
...
因为公理三、五和六所描述的内容，从数学上来讲要求效用集合是凸集。
所以，我们下面证明所定义的资源分配的效用向量集合~$\mathbf{\phi}$~是凸集（convex set)。
