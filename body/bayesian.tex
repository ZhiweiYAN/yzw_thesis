...
\label{ eqn_onlyone_equilibrium_cost }
\end{align}
%%%%%%%%%%%%%%
\section{数值分析与讨论}
...
\end{align}
我们把公式\ref{eqn_probability_of_contribution}代入公式\ref{eqn_equilibrium_prob}则有
\begin{align*} 
    c^* &= b - \tau + (1-c^*)^{N-1}\tau
\end{align*}
...
\end{align*}
服从正态分布的随机变量的概率密度函数$\f(c)$由下式给出，
\begin{align*}
    f(c) = \frac{1}{\sigma \sqrt{2\pi} } e^{ \frac{-(c-\mu)^2}{2\sigma^2}}
\end{align*}<++>
