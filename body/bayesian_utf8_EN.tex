...
If a suitable game model with rules is built, it can guide the users in the model to make their decisions, which benefits themselves and other's opponents.
...
There is a central unit to exchange the information and messages between the players of the game.
For instance, in the paper \cite{ZhangLiu2011}, all private information of players and parameters of payoff functions are transferred among users. 
...
However, the assumption is too ideal to describe other practical problems. 
Because the class of traffic are becoming complicated, and  the accurate information of players is hardly acquired by each player.
To handle the kind of problems, we introduce the Bayesian game theory to solve the problems.

In the paper, we construct a new game model, called Bayesian game analysis model. 
The model can guide the behavior of players under an incomplete information situation. 
Each player makes his reasonable decision, although they do not acquire the whole and complete information of other players in the game.

\section{Traffic Type and Game Model}
\subsection{Discrete and Continuous Traffic Class}
To our knowledge, the past research addresses the traffic class as a discrete form.
For example, the voice data is almost only one traffic over GSM networks. 
And the other media data appear over the communication systems. 
...
In summary, as a view of statistic theory, all traffic classes can construct a class set or a class space $Omega$. 
...
And the various traffic transport results are also obtained using different adaptive modulation coding or wireless channel models.

Therefore, we consider that it can be extended from the discrete definition form of traffic class to a continuous definition form.
That is, the class of each traffic can be viewed as a random variable $X$, which satisfies a statistical distribution $F(x)$.
The value of the random variable can correspond to a practical traffic class. 
And then we apply the continuous traffic class definition into an analysis model of Bayesian game, and examine the effects of this new definition on system resource allocation.
In the following sections, we will construct and analyze the proposed analysis model of Bayesian game.


\subsection{Bayesian Game Model}
We assume that there are $N$ users or players who are sharing the resource provided by a communication system.
