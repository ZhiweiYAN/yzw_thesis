...
\chapter{基于多媒体特性的呼叫接纳控制算法}
\echapter{Call Admission Control Algorithm Based on Multimedia Characteristics}
\label{chap_cacop}
...
这有两个原因：一个是以往无线网络承载的业务基本上以话音为主。其它的业务量的比重极小，几乎可以忽略不计。
二是虽然交叉层技术可以使得网络底层直接取得到上层（如应用层）的部分信息，但是这样做的结果会破坏目前无线网络分层结构设计原则。
这使得交叉层技术的应用范围大打折扣。
本文的工作就是围绕这一问题展开的。
...
\esection{ Analysis of Traffic QoS and CAC Model}
\label{sec_qos_metric}
因为不同的业务数据对服务质量的要求差别很大，所以要设计相应的传输机制来应对这种变化。
本节我们将详细描述如何将接纳控制算法、用户的QoS以及资源分配方案之间的关系。
...
（因为名词“呼叫”一般专指话音通信，而本文中的呼叫是以多种不同类型的数据流。所以，为避免文字概念混淆，下文一律用“连接（Connection)”一词来代替“呼叫(Call)”一词。）
...
首先，服务质量评估单元（QoS Evaluator）会收集目前的在线连接状况，记为向量 ~$\mathbf{b} = (b_1, b_2, \dots, b_K)$~ 。
...
\esubsection{Analysis of QoS and Resource Allocation}
...
\section{资源分配的优化分析}
\esection{Optimum Analysis of Resource Allocation}
...
%
\section{提出的接纳控制算法}
\esection{Proposed CAC algorithm}
\label{sec_alg}
...
\figref{fig:chap_cacop:clock_onging_call_sum} 则显示了在线连接的容量数目，可以看出，ElKadi方法与本文所提算法曲线非常接近，且明显优于基准算法。
