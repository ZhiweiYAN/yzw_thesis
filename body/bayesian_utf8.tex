...
然后，我们定义博弈中的参与者收益。
从前面的叙述中可知，每一个自私的参与者希望别的参与者选择“慷慨”，
...
但是，为了鼓励参与者选择“慷慨”，我们定义，如果所有参与者都选择“自私”，不愿意“牺牲”自己的利益，那么他们的获益~$b$~为~$0$~。
...
其中，我们把~$c_i^*$~称为均衡临界成本。
所以，我们需要求得参与者~$i$~选择“慷慨”概率，也就是均衡临界成本的概率~$P(c_i^*)$~。
\begin{align}
    P(c_i^*) = \hbox{Prob} \Bigl\{ C_{\min} < c_i \leq c_i^* \Bigr\} 
    \label{eqn:chap_bayesian:equil_prob_c_i}
\end{align}
而且，
因为对于所有用户的成本随机变量而言，是独立同分布。
所以，如果存在这样一个均衡临界成本，
可以将其视~$\theta_{-i}(m)$~为一个二项分布的情况。
...
所以存在一个唯一的~$c^*$~，且满足任一个\eqref{eqn_equilibrium_cost}，
...
另外，
从\figref{fig:bayesian_puni_para_vs_cont_prob}可以看到，当惩罚系数的值增大时，并不会让参与者决策均衡中“慷慨”的机率增大。这一点与我们的直觉是不同的。
