...
在这个仿真模型中，包括一个基站资源控制与分配单元和一个用户产生器。
基站所提供的全部资源总量为$C$。
用户产生器，可以按照预设的业务类型分布产生不同类型$c$的用户。
为了模拟系统的行为，仿真实验设置了以下几种情境：
\subsection{负载饱和情境}
为了模拟系统在负载饱和状态下的性能，用户产生器每次产生的用户对资源需求之和会大于资源供给的总量。
因为每一用户的类型值$c$与其资源申请的数量存在一一对应关系，我们此处将用户的类型值视为资源的申请量的一个表征值。
我们考察当资源需求总和超过资源供经总量的$10\%$,$20\%$和$30\%$的情况。
同时，仿真程序运行多次以得到一个统计意义的结果。
